...
const getData = async (offset: number, limit: number) => {
  let rs = await doSql("clwkbz1", offset, limit)
  const data: DataItem[] = []
  let tmpdata = rs.data_response.clwkbz1
...
const deleteData = async(key: string) => {
  ...
  let rs = await deleteRows('clwkbz1',0,10,drow[0].id)
  console.log(rs);
  if(rs.message=='success'){
...
const getDataWithKey = async(offset:number,limit:number)=>{
  let rs = await whereSql('clwkbz1',offset,limit,'id asc',`uuid:lk:'${keywords.value}'`)
    const data: DataItem[] = []
    let tmpdata = rs.data_response.clwkbz1
    tmpdata.forEach((item: any) => {
      item.key = item.id
