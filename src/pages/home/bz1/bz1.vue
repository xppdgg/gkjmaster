...
import {updatewkbz1,addwkbz1} from '../../../api/axios/clwkbz1/crud'
...
const getData = async (offset: number, limit: number) => {
  let rs = await doSql("clwkdatatrans", offset, limit)
  const data: DataItem[] = []
  let tmpdata = rs.data_response.clwkdatatrans
...
const deleteData = async(key: string) => {
  ...
  let rs = await deleteRows('clwkdatatrans',0,10,drow[0].id)
  console.log(rs);
  if(rs.message=='success'){
...
const getDataWithKey = async(offset:number,limit:number)=>{
  let rs = await whereSql('clwkdatatrans',offset,limit,'id asc',`uuid:lk:'${keywords.value}'`)
    const data: DataItem[] = []
    let tmpdata = rs.data_response.clwkdatatrans
    tmpdata.forEach((item: any) => {
      item.key = item.id
...
<template>
  <div style="background-color:  #007b52;height: 40px;display: flex;justify-content: space-between;padding: 0 30px;">
    <div style="width: 200px;display: flex;">
      <div style="color: white;line-height: 40px;margin-right: 10px;">协议</div>
