...
import {updatewkline,addwkline} from '../../../api/axios/clwkline/crud'
...
const getData = async (offset: number, limit: number) => {
  let rs = await doSql("clwkline", offset, limit)
  const data: DataItem[] = []
  let tmpdata = rs.data_response.clwkline
...
const save = async(key: string) => {
  ...
  let rs = await updatewkline(data)
  // console.log(369,rs);
...
const deleteData = async(key: string) => {
  ...
  let rs = await deleteRows('clwkline',0,10,drow[0].id)
  console.log(rs);
  if(rs.message=='success'){
...
const getDataWithKey = async(offset:number,limit:number)=>{
  let rs = await whereSql('clwkline',offset,limit,'id asc',`uuid:lk:'${keywords.value}'`)
    const data: DataItem[] = []
    let tmpdata = rs.data_response.clwkline
    tmpdata.forEach((item: any) => {
      item.key = item.id
...
<template>
  <div style="background-color:  #007b52;height: 40px;display: flex;justify-content: space-between;padding: 0 30px;">
    <div style="width: 200px;display: flex;">
      <div style="color: white;line-height: 40px;margin-right: 10px;">生产线</div>
  ...
  <a-table :columns="columns" :data-source="dataSource" bordered :pagination="pagination" @change="changePage" :scroll="{ x: 1500, y: 'calc(100vh - 223px)' }"
    ...
    <template v-slot:bodyCell="{ column, record, text, index }">
      ...
      <template v-for="col in ['uuid','name', 'company_id', 'clwkgroup_id', 'train_data', 'train_result']">
