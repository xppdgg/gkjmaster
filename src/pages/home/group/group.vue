...
import {updatewkgroup,addwkgroup} from '../../../api/axios/clwkgroup/crud'
...
const getData = async (offset: number, limit: number) => {
  let rs = await doSql("clwkgroup", offset, limit)
  const data: DataItem[] = []
  let tmpdata = rs.data_response.clwkgroup
...
const save = async(key: string) => {
  ...
  let rs = await updatewkgroup(data)
  // console.log(369,rs);
...
const deleteData = async(key: string) => {
  ...
  let rs = await deleteRows('clwkgroup',0,10,drow[0].id)
  console.log(rs);
  if(rs.message=='success'){
...
const getDataWithKey = async(offset:number,limit:number)=>{
  let rs = await whereSql('clwkgroup',offset,limit,'id asc',`uuid:lk:'${keywords.value}'`)
    const data: DataItem[] = []
    let tmpdata = rs.data_response.clwkgroup
    tmpdata.forEach((item: any) => {
      item.key = item.id
...
<template>
  <div style="background-color:  #007b52;height: 40px;display: flex;justify-content: space-between;padding: 0 30px;">
    <div style="width: 200px;display: flex;">
      <div style="color: white;line-height: 40px;margin-right: 10px;">车间</div>
