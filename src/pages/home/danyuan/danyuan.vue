...
import {updatewkdanyuan,addwkdanyuan} from '../../../api/axios/clwkdanyuan/crud'
...
const getData = async (offset: number, limit: number) => {
  let rs = await doSql("clwkdanyuan", offset, limit)
  const data: DataItem[] = []
  let tmpdata = rs.data_response.clwkdanyuan
...
const save = async(key: string) => {
  ...
  let rs = await updatewkdanyuan(data)
  // console.log(369,rs);
  ...
  let rs = await addwkdanyuan(data)
  // console.log(369,rs);
  if(rs.message=='success'){
...
const deleteData = async(key: string) => {
  ...
  let rs = await deleteRows('clwkdanyuan',0,10,drow[0].id)
  console.log(rs);
  if(rs.message=='success'){
...
const getDataWithKey = async(offset:number,limit:number)=>{
  let rs = await whereSql('clwkdanyuan',offset,limit,'id asc',`uuid:lk:'${keywords.value}'`)
    const data: DataItem[] = []
    let tmpdata = rs.data_response.clwkdanyuan
    tmpdata.forEach((item: any) => {
      item.key = item.id
...
<template>
  <div style="background-color:  #007b52;height: 40px;display: flex;justify-content: space-between;padding: 0 30px;">
    <div style="width: 200px;display: flex;">
      <div style="color: white;line-height: 40px;margin-right: 10px;">单元</div>
