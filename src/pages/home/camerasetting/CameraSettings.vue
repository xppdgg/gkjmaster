<template>
    ...
    <a-modal title="修改摄像头数据" v-model:open="updateOpen" @ok="() => { updateSensor }" :footer="[]" wrapClassName="full-modal">
...
import UpdateCamera from '../../../components/cameraDataconf/UpdateCamera.vue';
const columns: TableColumnsType = [
    ...
    { title: '数据', dataIndex: 'name', key: 'name', width: 150 },
...
const getCamera = async (offset:number,limit:number) => {    
    // console.log(423,route);
...
const jumpNew = (param:string)=>{
    const newpage = router.resolve({ name: 'cldataimage',params:{databaseName:param,companyid:userii.companyid as number,roleid:userii.roleid as number,userid:userii.userid as number} });
...
const changePage = (pageinfo:{current:number, pageSize: number,showQuickJumper:boolean,showSizeChanger:boolean,total: number,})=>{
    ...
    getCamera((current-1)*10,pageSize)
}
</script>
